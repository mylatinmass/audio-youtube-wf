...
def parse_search_pattern(pattern):
    ...
    Supports:
      - Optional words via trailing '?'.
      - Alternatives via '|' in the token.
    """
...
def match_pattern(words, i, pattern, p_index):
    ...
    if p_index == len(pattern):
        return i
    # No more words: only succeed if all remaining pattern tokens are optional
    if i >= len(words):
        for j in range(p_index, len(pattern)):
            if not pattern[j]['optional']:
                return None
        return i
    ...
    # Optional token: try skipping it first, then consuming if it matches
    if pat['optional']:
...
def find_phrase_timestamps(transcript, search_phrase, backwards=False, skip=0.0):
    ...
    of search_phrase in the transcript. Returns (None, None) if not found.
    """
...
def find_homily(transcript):
    """
    Extract everything from the FIRST "Holy Ghost" (skip its Amen) up through
    the LAST "Holy Ghost" (include its Amen if present).
    Returns: (first, last, homily_text, video_segments)
    """
    # Optional Amen with '?'
    marker = "holy ghost amen?"

    # first occurrence (forward)
    first, _ = find_phrase_timestamps(transcript, marker, backwards=False)
    if first is None:
        raise RuntimeError("Opening 'Holy Ghost' not found")

    # last occurrence (backward)
    _, last = find_phrase_timestamps(transcript, marker, backwards=True)
    if last is None:
        raise RuntimeError("Closing 'Holy Ghost' not found")

    # build trimmed segments
    trimmed = []
    for seg in transcript.get('segments', []):
        words = [w for w in seg['words'] if w['start'] >= first and w['end'] <= last]
        if not words:
            continue
        trimmed.append({
    ...
    video_segments = [(seg['start'], seg['end'], seg['text']) for seg in trimmed]
