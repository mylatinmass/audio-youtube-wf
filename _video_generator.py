...
def generate_subtitle_overlay_clips(segments, video_size):
    ...
    total_duration = segments[-1][1]  # overall video duration from transcript
    ...
    for i, (start, end, sentence) in enumerate(segments):
        # For the very first segment, force the clip to start at 0 so there is no gap.
        if i == 0:
            actual_start = 0
            if num_segments > 1:
        ...
        # Create the background image and overlay.
    ...
    # Append the outro clip (generated with your outro function).
...
def generate_subtitle_background_image_intro(video_size, duration, fps=24):
    """
    Generates frames for an intro clip that lasts from 0:00 until the first text clip.
    For the first 1 second, the overlay alpha transitions from 0 to 150.
    After 1 second, the overlay remains at alpha 150.
    """
    bg_img = Image.open(background_image_path).convert("RGBA")
    bg_img = bg_img.resize(video_size, Image.Resampling.LANCZOS)
    ...
    for i in range(num_frames):
        current_time = i / fps  # current time in seconds
        if current_time <= 1:
            # Linear transition from alpha 0 to 150 over the first second
...
def filter_homily_segments(transcript_json, homily_start, homily_end):
    """
    Reads the full transcript JSON file, filters the segments to include only those within
    the homily's start and end timestamps, and writes the filtered segments to a new JSON file.
    Returns the path to the filtered JSON.
    """
    with open(transcript_json, "r", encoding="utf-8") as f:
        data = json.load(f)
    
    # Filter segments: include only those that start and end within the homily times.
    filtered_segments = [
...
def create_text_video(json_path, bg_img_path, audio_path, video_size=(video_width, video_height)):
    # Override global file paths with the ones provided as arguments
    ...
    try:
        ...
        #-------- FOR TESTING ONLY: LIMIT VIDEO TO 60 SECONDS -------
        print("Limiting video to 60 seconds for testing...")
        final_video = final_video.with_duration(60)
        

        #-------- TO RENDER THE FULL VIDEO -------
        # print("Outputing final video...")
        # final_video = final_video
        
        print("Loading and integrating audio clip...")
        silence = AudioClip(lambda t: 0, duration=1, fps=44100)
        audio_clip = AudioFileClip(audio_file_path).with_start(1) 
        delayed_audio = concatenate_audioclips([silence, audio_clip])
        final_video = final_video.with_audio(delayed_audio)

        output_path = os.path.join(os.path.dirname(json_path), "final-video.mp4")
        print("Writing video file... This may take a while.")
        final_video.write_videofile(output_path, fps=24, codec="libx264", logger="bar")
...
if __name__ == "__main__":
    # For testing, we call create_text_video with cleaned default paths.
    create_text_video(
